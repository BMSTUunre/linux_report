...
- Отчет:
	...
	3. Оглавление заменить на СОДЕРЖАНИЕ. Более того, сделать обязательные разделы:
	```ВВЕДЕНИЕ, СЦЕНАРИЙ КОМАНДНОЙ ОБОЛОЧКИ, ЗАКЛЮЧЕНИЕ, СПИСОК ИСПОЛЬЗОВАННЫХ ИСТОЧНИКОВ.```
...
- Правки от костра:
     ...
     1. ВВЕДЕНИЕ - без воды, конкретные цель и задачи. Воду лить не стоит.

     2. ТЕРМИНЫ И ОПРЕДЕЛЕНИЯ. Если вы используете термины, например ( Linux, Bash, Скрипт и т.д.), то должны дать им определения.   
       Термины и определения - идут после оглавления. Определения брать только из авторитетных источников (книги и проверенные сайты).  
       Источник откуда вы взяли определения прописать и в «ТЕРМИНЫ И ОПРЕДЕЛЕНИЯ» и в «СПИСОК ИСПОЛЬЗОВАННЫХ ИСТОЧНИКОВ»

    3. ПЕРЕЧЕНЬ СОКРАЩЕНИЙ И ОПРЕДЕЛЕНИЙ. Если имеете хоть одно сокращение (например ЧПТ) выносите в перечень описанный выше. Он идет после «ТЕРМИНЫ И ОПРЕДЕЛЕНИЯ» 

    4. ОПИСАНИЕ СКРИПТОВ. 
		1. Нельзя писать в таком виде:
    		>   Вариант №1
    	Так лучше не делать.Использовать как можно меньше двоеточий.   
    
		2. Листинг оформлять как рисунок ( он написан текстом, но оформлять только как рисунок,  
			в ГОСТЕ можно найти инфу). Запрещено нарушать границы таблицы,пример ошибки:
        ...
        Таблицы на каждой странице должны быть закрыты, а именно с четкими границами и сверху и снизу.
	
        ‼️ВАЖНО. Если листинг comparator*.sh не вмещается - его МОЖНО перенести, тк он нужен нам здесь и сейчас. Если не вмещается вывод test.sh, то его ПЕРЕНОСИТЬ в приложение, оформляя по госту.Шрифт скрипта НЕ МЕНЬШЕ 14ого размера, он должен быть везде одинаковый по размеру, те для comparator1.sh, comparator2.sh и тд 

        5. ТАБЛИЦЫ С ИТОГАМИ. Оформлять СТРОГО ПО ГОСТУ И ТОЛЬКО, подписывать ее тоже по ГОСТУ

       6. ЗАКЛЮЧЕНИЕ. Без воды, соотвествующие задачам и целям в ВВЕДЕНИИ. Писать только то, что вы сделали, те конкретно что вы сделали, без 
       лишнего

       7. ИСТОЧНИКИ. Только 2 источника, а именно сайт e-learning, добавить книгу, про которую писал Александр Сергеевич, но можно и без нее.
       Ну и получается ( не уверен в этом) что ссылки на источники с которых вы брали определения ( смотреть п.2 )


- ОБЩИЕ ПОМЕТКИ:
	1. Получить знания и … - низя
	2. "" - низя, только «» (исключение листинг кода)
	3. Стараться избегать двоеточий ":", в минимальном количестве допускаются